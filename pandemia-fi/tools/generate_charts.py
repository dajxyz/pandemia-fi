...
def main(args):
    ...
    if args.save_backup:
        p = os.path.join("data_backup", "ecdc/", "ecdc-%s.json" %
    ...
    selected_countries = ["Finland", "Sweden", "Norway", "Estonia", "Denmark", "Russia",
                          "Italy", "Spain", "France", "Germany", "United_Kingdom", "United_States_of_America"]
    ...
    for entry in ecdc["records"]:
        if entry["countriesAndTerritories"] in world:
            ...
            try:
                ...
                popData2018 = None
                logging.debug(
                    "Population data missing for country '%s'" % country)

            d = datetime.date(year, month, day)
            entry = {"date": d, "cases": cases,
                     "deaths": deaths, "popData2018": popData2018}
            ...
            for a in areas.keys():
                if country in areas[a]:
                    if d not in area_stats[a]["entries"].keys():
                        area_stats[a]["entries"][d] = {
                            "date": d, "cases": cases, "deaths": deaths, "popData2018": popData2018}
                    else:
    ...
    for c in world:
        country_stats[c]["entries"] = sorted(
            country_stats[c]["entries"], key=lambda x: x["date"])
    ...
    for c in selected_countries:
        ...
        for e in country_stats[c]["entries"]:
            if e["date"] >= datetime.date(2020, 2, 21):
                ...
                if e["cum_cases"] > 0:
                    s += "{x: '%s GMT', y: %f}, " % (e["date"].strftime(
                        "%-m-%-d-%Y"), (e["cum_cases"] / e["popData2018"]) * 50000 * 100)
                else:
                    s += "{x: '%s GMT', y: null}, " % (
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Generate charts for pandemia.fi site.')
